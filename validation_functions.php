...
function verifyTrack ($testString) {
    // Check for letters, numbers and dash, period, space, single quote, and parentheses only. 
    return (preg_match ("/^([[:alnum:]]|-|\.| |'|\)|\()+$/", $testString));
}

function verifyProducer ($testString) {
    // Check for letters, numbers and dash, period, space, single quote, ampersand, and commas only. 
    return (preg_match ("/^([[:alnum:]]|-|\.| |'|&|,)+$/", $testString));
}

function verifyGenre ($testString) {
    // Check for letters and spaces only.
    return (preg_match ("/^([[:alpha:]]| )+$/", $testString));
}

function verifyKey ($testString) {
    // Check for letters only, # when necessary.
    return (preg_match ("/^([[:alpha:]]|#)+$/", $testString));
}

/*
function verifyMediaURL ($testString) {
    // Check for media link correctness necessary.
    return (preg_match ("/^([[:alnum:]]|:|-|\.|=|?|&)+$/", $testString));
}
*/
